...
df_filtered["Player"] = df_filtered["Player"].str.replace(
    r"( - \w+).*$", r"\1", regex=True
)

# Filter rows where "[Year] Draft Position" starts with "Round"
df_filtered = df_filtered[
    df_filtered["[Year] Draft Position"].str.startswith("Round", na=False)
].copy()

# Extract the number after "Round" and create a new column "[Year] Position Number"
df_filtered.loc[:, "[Year] Position Number"] = (
    df_filtered["[Year] Draft Position"].str.extract(r"Round (\d+)").astype(int)
)
